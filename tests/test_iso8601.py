...
from nr.date import ISO_8601, duration

ISO8601_DATETIME_SAMPLES = SAMPLES[
    SAMPLES.tags.apply(lambda x: "iso8601" in x) & SAMPLES.parsed.apply(lambda x: isinstance(x, datetime.datetime))
]

ISO8601_DURATION_SAMPLES = SAMPLES[
    SAMPLES.tags.apply(lambda x: "iso8601" in x) & SAMPLES.parsed.apply(lambda x: isinstance(x, duration))
]
